...
class OrderFeedback extends Notification
{
    use Queueable;
    ...
    public function toArray($notifiable)
    {
        ...
        if($notifiable->hasAnyRole('JobSeeker')){
            return [
                'heading' => 'Service Order – Feedback',
                'text' => "Successfully accomplished Feedback Service Order for ' {$order->service->title} ' with No.: {$order_id}"
            ];
        }elseif($notifiable->hasAnyRole('Backer')){
            return [
                'heading' => 'Service Order – Feedback',
                'text' => "Successfully accomplished Feedback Service Order for ' {$order->service->title} ' by {$jobseeker_name} with No.: {$order_id}"
            ];
        }
    }
}
